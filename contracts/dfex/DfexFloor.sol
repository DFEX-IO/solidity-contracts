...
contract DfexFloor is ReentrancyGuard, TokenManager {
    ...
    modifier onlyHandler() {
        require(isHandler[msg.sender], "DfexFloor: forbidden");
        _;
    }
    ...
    function setBackedSupply(uint256 _backedSupply) public onlyAdmin {
        require(_backedSupply > backedSupply, "DfexFloor: invalid _backedSupply");
        backedSupply = _backedSupply;
    }

    function setMintMultiplier(uint256 _mintMultiplier) public onlyAdmin {
        require(_mintMultiplier > mintMultiplier, "DfexFloor: invalid _mintMultiplier");
        mintMultiplier = _mintMultiplier;
    }

    // mint refers to increasing the circulating supply
    // the DFEX tokens to be transferred out must be pre-transferred into this contract
    function mint(uint256 _amount, uint256 _maxCost, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        require(_amount > 0, "DfexFloor: invalid _amount");
        ...
        require(cost <= _maxCost, "DfexFloor: _maxCost exceeded");
        ...
        IERC20(dfex).transfer(_receiver, _amount);
    ...
    function burn(uint256 _amount, uint256 _minOut, address _receiver) public onlyHandler nonReentrant returns (uint256) {
        ...
        require(amountOut >= _minOut, "DfexFloor: insufficient amountOut");

        backedSupply = backedSupply.sub(_amount);

        IMintable(dfex).burn(msg.sender, _amount);
