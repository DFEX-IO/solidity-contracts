...
import "./interfaces/IDfexIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IDfexMigrator.sol";

contract DfexMigrator is ReentrancyGuard, IDfexMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "DfexMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "DfexMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "DfexMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "DfexMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "DfexMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "DfexMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "DfexMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "DfexMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        dfexPrice = _dfexPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "DfexMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "DfexMigrator: token not whitelisted");
        require(_tokenAmount > 0, "DfexMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "DfexMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(dfexPrice);
        require(mintAmount > 0, "DfexMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "DfexMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "DfexMigrator: invalid tokenA");
            require(tokenB != address(0), "DfexMigrator: invalid tokenB");
        ...
        IDfexIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "DfexMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "DfexMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "DfexMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "DfexMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("DfexMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "DfexMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "DfexMigrator: invalid _action");
