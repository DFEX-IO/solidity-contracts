...
contract Vault is ReentrancyGuard, IVault {
    ...
    // usddAmounts tracks the amount of USDD debt for each whitelisted token
    mapping (address => uint256) public override usddAmounts;

    // maxUsddAmounts allows setting a max amount of USDD debt for a token
    mapping (address => uint256) public override maxUsddAmounts;
    ...
    // this value is used to calculate the redemption values for selling of USDD
    ...
    event BuyUSDD(address account, address token, uint256 tokenAmount, uint256 usddAmount, uint256 feeBasisPoints);
    event SellUSDD(address account, address token, uint256 usddAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    event IncreaseUsddAmount(address token, uint256 amount);
    event DecreaseUsddAmount(address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        maxUsddAmounts[_token] = _maxUsddAmount;
    ...
    function setUsddAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 usddAmount = usddAmounts[_token];
        if (_amount > usddAmount) {
            _increaseUsddAmount(_token, _amount.sub(usddAmount));
            return;
        }

        _decreaseUsddAmount(_token, usddAmount.sub(_amount));
    }
    ...
    // deposit into the pool without minting USDD tokens
    ...
    function buyUSDD(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        uint256 usddAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        usddAmount = adjustForDecimals(usddAmount, _token, usdd);
        _validate(usddAmount > 0, 18);

        uint256 feeBasisPoints = vaultUtils.getBuyUsddFeeBasisPoints(_token, usddAmount);
        uint256 amountAfterFees = _collectSwapFees(_token, tokenAmount, feeBasisPoints);
        uint256 mintAmount = amountAfterFees.mul(price).div(PRICE_PRECISION);
        mintAmount = adjustForDecimals(mintAmount, _token, usdd);

        _increaseUsddAmount(_token, mintAmount);
        _increasePoolAmount(_token, amountAfterFees);

        IUSDD(usdd).mint(_receiver, mintAmount);

        emit BuyUSDD(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellUSDD(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        _validate(whitelistedTokens[_token], 19);
        useSwapPricing = true;

        uint256 usddAmount = _transferIn(usdd);
        _validate(usddAmount > 0, 20);

        updateCumulativeFundingRate(_token, _token);

        uint256 redemptionAmount = getRedemptionAmount(_token, usddAmount);
        _validate(redemptionAmount > 0, 21);

        _decreaseUsddAmount(_token, usddAmount);
        _decreasePoolAmount(_token, redemptionAmount);

        IUSDD(usdd).burn(address(this), usddAmount);

        // the _transferIn call increased the value of tokenBalances[usdd]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for usdd, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(usdd);

        uint256 feeBasisPoints = vaultUtils.getSellUsddFeeBasisPoints(_token, usddAmount);
        uint256 amountOut = _collectSwapFees(_token, redemptionAmount, feeBasisPoints);
        _validate(amountOut > 0, 22);

        _transferOut(_token, amountOut, _receiver);

        emit SellUSDD(_receiver, _token, usddAmount, amountOut, feeBasisPoints);
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        // adjust usddAmounts by the same usddAmount as debt is shifted between the assets
        uint256 usddAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        usddAmount = adjustForDecimals(usddAmount, _tokenIn, usdd);

        uint256 feeBasisPoints = vaultUtils.getSwapFeeBasisPoints(_tokenIn, _tokenOut, usddAmount);
        uint256 amountOutAfterFees = _collectSwapFees(_tokenOut, amountOut, feeBasisPoints);

        _increaseUsddAmount(_tokenIn, usddAmount);
        _decreaseUsddAmount(_tokenOut, usddAmount);
    ...
    function getRedemptionAmount(address _token, uint256 _usddAmount) public override view returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _usddAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, usdd, _token);
    }
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == usdd ? USDD_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usdd ? USDD_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getFeeBasisPoints(address _token, uint256 _usddDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        return vaultUtils.getFeeBasisPoints(_token, _usddDelta, _feeBasisPoints, _taxBasisPoints, _increment);
    }

    function getTargetUsddAmount(address _token) public override view returns (uint256) {
        uint256 supply = IERC20(usdd).totalSupply();
    ...
    function _increaseUsddAmount(address _token, uint256 _amount) private {
        usddAmounts[_token] = usddAmounts[_token].add(_amount);
        uint256 maxUsddAmount = maxUsddAmounts[_token];
        if (maxUsddAmount != 0) {
            _validate(usddAmounts[_token] <= maxUsddAmount, 51);
        }
        emit IncreaseUsddAmount(_token, _amount);
    }

    function _decreaseUsddAmount(address _token, uint256 _amount) private {
        uint256 value = usddAmounts[_token];
        // since USDD can be minted using multiple assets
        // it is possible for the USDD debt for a single asset to be less than zero
        // the USDD debt is capped to zero for this case
        if (value <= _amount) {
            usddAmounts[_token] = 0;
            emit DecreaseUsddAmount(_token, value);
            return;
        }
        usddAmounts[_token] = value.sub(_amount);
        emit DecreaseUsddAmount(_token, _amount);
    }
