...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getBuyUsddFeeBasisPoints(address _token, uint256 _usddAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usddAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsddFeeBasisPoints(address _token, uint256 _usddAmount) public override view returns (uint256) {
        return getFeeBasisPoints(_token, _usddAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }

    function getSwapFeeBasisPoints(address _tokenIn, address _tokenOut, uint256 _usddAmount) public override view returns (uint256) {
        ...
        uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, _usddAmount, baseBps, taxBps, true);
        uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, _usddAmount, baseBps, taxBps, false);
    ...
    function getFeeBasisPoints(address _token, uint256 _usddDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        ...
        uint256 initialAmount = vault.usddAmounts(_token);
        uint256 nextAmount = initialAmount.add(_usddDelta);
        if (!_increment) {
            nextAmount = _usddDelta > initialAmount ? 0 : initialAmount.sub(_usddDelta);
        }

        uint256 targetAmount = vault.getTargetUsddAmount(_token);
