...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    uint256 public constant USDD_PRECISION = 1e18;
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _usdd, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getUsddMinPrice(address _otherToken) public view returns (uint256) {
        // USDD_PRECISION is the same as 1 USDD
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDD_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. USDD doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDD price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDD, BNB, BTC] we need to know how much BNB we'll get for provided USDD
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of USDD
        if (tokenA == usdd) {
            // with both _path.length == 2 or 3 we need usdd price against _path[1]
            tokenAPrice = getUsddMinPrice(_path[1]);
        } else {
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usdd) { // buyUSDD
            amountOut = IVault(vault).buyUSDD(_tokenIn, _receiver);
        } else if (_tokenIn == usdd) { // sellUSDD
            amountOut = IVault(vault).sellUSDD(_tokenOut, _receiver);
        } else { // swap
