...
import "./interfaces/IDfexTimelock.sol";
...
contract DfexTimelock is IDfexTimelock {
    ...
    event SignalRedeemUsdd(address vault, address token, uint256 amount);
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "DfexTimelock: forbidden");
        _;
    }

    modifier onlyAdminOrHandler() {
        require(msg.sender == admin || isHandler[msg.sender], "DfexTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "DfexTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "DfexTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "DfexTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "DfexTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "DfexTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "DfexTimelock: invalid _buffer");
        require(_buffer > buffer, "DfexTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }

    function setMaxLeverage(address _vault, uint256 _maxLeverage) external onlyAdmin {
      require(_maxLeverage > MAX_LEVERAGE_VALIDATION, "DfexTimelock: invalid _maxLeverage");
    ...
    function setFundingRate(address _vault, uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external onlyAdmin {
        require(_fundingRateFactor < MAX_FUNDING_RATE_FACTOR, "DfexTimelock: invalid _fundingRateFactor");
        require(_stableFundingRateFactor < MAX_FUNDING_RATE_FACTOR, "DfexTimelock: invalid _stableFundingRateFactor");
    ...
    function setFees(
        ...
        require(_taxBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _taxBasisPoints");
        require(_stableTaxBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _stableTaxBasisPoints");
        require(_mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _mintBurnFeeBasisPoints");
        require(_swapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _swapFeeBasisPoints");
        require(_stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _stableSwapFeeBasisPoints");
        require(_marginFeeBasisPoints < MAX_FEE_BASIS_POINTS, "DfexTimelock: invalid _marginFeeBasisPoints");
        require(_liquidationFeeUsd < 10 * PRICE_PRECISION, "DfexTimelock: invalid _liquidationFeeUsd");
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "DfexTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(vault.whitelistedTokens(_token), "DfexTimelock: token not yet whitelisted");
        ...
        IVault(_vault).setUsddAmount(_token, _usddAmount);
    }
    ...
    function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode) external onlyAdmin {
        if (excludedTokens[_token]) {
            ...
            require(_inPrivateTransferMode == false, "DfexTimelock: invalid _inPrivateTransferMode");
        }
    ...
    function signalRedeemUsdd(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemUsdd", _vault, _token, _amount));
        _setPendingAction(action);
        emit SignalRedeemUsdd(_vault, _token, _amount);
    }

    function redeemUsdd(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemUsdd", _vault, _token, _amount));
        _validateAction(action);
        _clearAction(action);

        address usdd = IVault(_vault).usdd();
        IVault(_vault).setManager(address(this), true);
        IUSDD(usdd).addVault(address(this));

        IUSDD(usdd).mint(address(this), _amount);
        IERC20(usdd).transfer(address(_vault), _amount);

        IVault(_vault).sellUSDD(_token, mintReceiver);

        IVault(_vault).setManager(address(this), false);
        IUSDD(usdd).removeVault(address(this));
    }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "DfexTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "DfexTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "DfexTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "DfexTimelock: invalid _action");
