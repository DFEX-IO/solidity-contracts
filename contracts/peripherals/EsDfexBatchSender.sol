...
contract EsDfexBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esDfex;

    constructor(address _esDfex) public {
        admin = msg.sender;
        esDfex = _esDfex;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsDfexBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esDfex).transferFrom(msg.sender, _accounts[i], _amounts[i]);
