...
contract Reader is Governable {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxUsddAmount = _vault.maxUsddAmounts(_tokenIn);

        if (maxUsddAmount != 0) {
            if (maxUsddAmount < _vault.usddAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsddAmount.sub(_vault.usddAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** USDD_DECIMALS);
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 usddAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usddAmount = usddAmount.mul(10 ** USDD_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usddAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usddAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _usddAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usddAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usddAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _usddAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsddAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _usddAmount, address[] memory _tokens) public view returns (uint256[] memory) {
